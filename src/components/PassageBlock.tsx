 ...
 * - Arabic (RTL) auto-detection per paragraph — paragraf berbahasa Arab
 *   otomatis rata kanan dengan nomor paragraf di sisi kanan
            ...
            <div className="p-4 space-y-0">
                {isMultiParagraph && showParagraphNumbers ? (
                    ...
                    paragraphs.map((para, idx) => {
                        const dir = detectTextDirection(para)
                        return (
                            <div
                                key={idx}
                                dir={dir}
                                className={`flex gap-3 ${dir === 'rtl' ? 'flex-row-reverse' : ''} ${idx > 0 ? 'mt-4 pt-4 border-t border-teal-200/50 dark:border-teal-700/50' : ''}`}
                            >
                    ...
                    // Single paragraph: render as-is with SmartText (direction auto-detected)
                    <div dir={detectTextDirection(text)}>
                        <SmartText
                            text={text}
