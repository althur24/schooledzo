...
import {
    ...
    ChevronDown, FileSpreadsheet, FileText, Info
} from 'lucide-react'
...
function SiswaPageContent() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true)
            try {
                const url = urlClassId ? `/api/guru/siswa?class_id=${encodeURIComponent(urlClassId)}` : '/api/guru/siswa'
    ...
    const handleExport = async (format: 'xlsx' | 'csv') => {
        ...
        try {
            ...
            if (format === 'xlsx') {
                ...
                XLSX.writeFile(wb, `rekap_siswa_${safeClassName}.xlsx`)
            } else {
    ...
    if (!data || !data.classes || data.classes.length === 0) {
        return (
            <div className="space-y-6">
                <PageHeader
                    title="Siswa"
                    subtitle="Performa akademik siswa di kelas Anda"
                ...
                <Card className="p-6">
                    <EmptyState
                        ...
                        description="Anda belum ditugaskan mengajar atau menjadi wali kelas manapun"
                    />
                </Card>
            </div>
        )
    ...
    const isHomeroom = currentClass?.isHomeroom ?? data.is_homeroom ?? false
    ...
    return (
        <div className="space-y-6">
                    ...
                    {data.classes.map((cls: any) => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex-shrink-0 flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold border transition-all ${active
                                    ? 'bg-primary text-white border-primary shadow-md shadow-primary/20'
                                    : 'bg-white dark:bg-surface-dark text-text-main dark:text-white border-secondary/20 dark:border-white/10 hover:border-primary/40'
                                    }`}
                            >
                                {cls.name}
                                {cls.isHomeroom && (
            ...
            {/* Scope notice for non-homeroom classes */}
            {!isHomeroom && (
                <div className="flex items-center gap-2.5 p-3 rounded-2xl bg-blue-50/70 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30">
                    <Info className="w-4 h-4 text-blue-500 flex-shrink-0" />
                    <p className="text-xs font-bold text-blue-700 dark:text-blue-400">
                        Anda bukan wali kelas di kelas ini — hanya menampilkan mata pelajaran yang Anda ampu.
                    </p>
                </div>
            )}
                            ...
                            {filteredStudents.map((student: any, idx: number) => {
                                ...
                                return (
                                    <tr
                                        ...
                                        onClick={() => router.push(`/dashboard/guru/siswa/${student.id}?class_id=${data.current_class_id}`)}
                                    >
                                        ...
                                        <td className="px-4 py-3 sticky left-10 bg-white dark:bg-surface-dark group-hover:bg-secondary/5 z-10">
                                            <div className="flex items-center gap-2">
                                                <div className="w-8 h-8 rounded-full bg-gradient-to-r from-blue-500 to-primary flex items-center justify-center text-white text-xs font-bold">
