...
export async function GET(request: NextRequest) {
    try {
        ...
        if (quizId) {
            // Tenant guard: quiz harus milik sekolah caller (param client dipercaya)
            if ((await findQuizzesOutsideSchool([quizId], schoolId)).length > 0) {
        ...
        if (!quizId && allYears !== 'true') {
            ...
            if ((activeYears || []).length > 1) {
                console.warn(`[quiz-submissions] Sekolah ${schoolId} punya ${activeYears!.length} tahun aktif — pakai yang terbaru`)
            }
