...
- **Load test via Railway staging (terbukti 25 Sep 2026)**: `LOAD_BASE=https://schooledzo-production-e036.up.railway.app` di `load_login`/`load_smoke` (server remote, tanpa spawn lokal; `assertServerDb` memverifikasi target menunjuk DB staging). k6: `k6 run -e BASE_URL=https://... -e FAST=1` + `loadtest/e2e/monitor_poll.cjs` jalan paralel (guru polling Monitor Live tiap 15 dtk selama 1000 VU — bukti sisi server, lihat bawah). E2E manual (`scripts/e2e-*-staging.cjs`) juga bisa remote via `E2E_BASE=https://...`.
- **Hindari spawn lokal untuk beban ≥300**: `kern.ipc.somaxconn=128` (default macOS) RST koneksi herd serentak (mini-repro: 300 concurrent → 164 ECONNRESET di server HTTP trivial). `start_exam` p95 lokal 10 dtk vs 429 ms via Railway untuk beban yang sama. Server `next start` lokal hanya utk E2E fungsional.
- **k6 dari laptop via hotspot: `http_req_failed` ±3-4% = artefak KLIEN, bukan aplikasi** (terbukti 25 Sep): pola bimodal — request sukses p95 225 ms/max 1 s, yang "gagal" stall cliff persis 60 s (timeout k6). Buktikan sisi server sehat dengan `monitor_poll.cjs` (42/42 poll 200, p95 1,2 s SELAMA run 1000 VU) + hantam endpoint langsung 1000 konkuren dari laptop (`node exp_tls_conc` pola: 1000/1000 sukses 6,5 s). 1000 koneksi TLS serentak dari laptop yang sama ke Supabase juga bersih → bottleneck = NAT hotspot utk flow tersustan, bukan server/Railway edge. Produksi (siswa tersebar, router sekolah, browser auto-retry) tak terdampak pola ini.
- **Verifikasi kode Railway staging = main HEAD**: jalankan `e2e-checkended` phase1 remote (`E2E_BASE`); phase2 butuh throttle in-process 10 mnt expire — di remote tak bisa restart server, TUNGGU >10 mnt setelah GET guru terakhir (TTL expiry setara restart).
- **Artefak re-run E2E staging**: migrasi data sekali-jalan (mis. `20260923035940` reaktivasi) TIDAK menyentuh fixture yang di-seed ulang — replikasi SQL-nya via service-role bila perlu menguji jalur "korban sweeper". Fixture load_smoke yang di-abort mid-run meninggalkan user `ld_%` + kelas/mapel/ujian ber-nama `ld_%` (cleanup di finally tak jalan) — bersihkan manual via service-role.
